...
public class SRPP extends SRSolver {
    ...
    boolean robust = true; // TODO parametrize
    ...
    public void solve(Setting setting, long milliseconds) {
        ...
        if (System.currentTimeMillis() < endTime) {
            if (scenarioChoice.equals("preprocess")) {
                ...
                uMax = solveILP(paths, root, topology, endTime); // TO CHANGE
            }
        }
    ...
    private double solveILP (ArrayList<int[]> paths, SegmentTreeRoot root, Topology topology, long endTime) {
        ...
        ArrayList<Double>[] edges = new ArrayList[topology.nEdges];
        ...
        for (int i = 0; i < paths.size(); i++) {
            ...
            for (EdgePair edgePair : edgeLoads) {
                if (edgePair.getLoad() != 0) {
                    ...
                    edges[edgePair.getKey()].add(root.trafficMatrix[startNode][endNode]*edgePair.getLoad());
                }
            }
        }
        double max = 0.0;
        for (int i = 0; i < topology.nEdges; i++) {
            double sum = 0;
            for (Double load : edges[i]) {
                sum += load;
            }
            if (robust) {
                edges[i].sort(Collections.reverseOrder());
                for (int j = 0; j < edges[i].size() && j < gammaParameter; j++) {
                    sum += edges[i].get(j);
                }
            }
